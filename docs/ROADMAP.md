...
| 8 | **Switch-detection drift signal** — advisory ✅ | **Advisory (voice supervisor) path now runs the switch router (`route_switch`) EVERY turn from turn 1, IN PARALLEL with classify/retrieval/embed** (no added wall-clock latency), judged on the latest utterance — the reliable drift signal the old non-empty-out-of-vocab heuristic wasn't. Verified: parking→flight switches on the turn it happens; a same-topic follow-up keeps (no over-switch). Tradeoff: one extra small router LLM call per advisory turn (parallel). The **non-advisory** paths still use the conservative vocab heuristic — unify if needed. | 🟠 |
...
| 14b | **Prompt blocks: stage-scoped application in advisory** ✅ done | Advisory now runs retrieval + classification + embedding in parallel, then generates the reply scoped to the **classified stage's** blocks only (resolved from the session-pinned bindings; entry-stage fallback on turn 0). Respond waits for `max(classify, retrieval)` — retrieval dominates, so no added latency. Verified: a ProvideInformation turn carries only `directions.confirm`, a VerifyWhatsNeeded turn only `verify.confirm` — not the whole SOP's set. | ✅ |
| 14c | **Prompt blocks in the onboarding config** ✅ done | The stage→block binding map now lives in the onboarding config (`prompt_bindings`: `{stage_matches, blocks, sop_matches?}` rules). `onboard.py` applies it on provision (both JSON-def and text-ingest paths); shipped SOP definitions are **binding-free** so the config is the single source of truth; `bind_prompts.py` reads the same config. Verified: a fresh tenant with binding-free SOPs gets all correct per-stage bindings from the config alone. A customer changes wording *or placement* by editing config — no code. | ✅ |
