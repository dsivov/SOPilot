...
| D-11 | **SOP routing lives inside SOPilot**, not in the caller's model: sessions may start SOP-less (**intake mode**); a cheap-LLM router (`router_model`, default gpt-4o-mini) routes on the first routable utterance and defers on greetings; a **switch check** runs only when the tracker loses the thread (classified state outside the current SOP's vocabulary); every decision (initial/defer/oos/switch) lands in `routing_events` with reason + latency; explicit `sop_id` from the integration always wins (IVR menu, deep link) | measured before built on the AENA corpus (600-opening oracle decomposition): 38% of real openings are not routable yet (→ defer is a first-class outcome), whole-dialogue labels are ~31% noisy at the opening, cheap router ≈ strong oracle on 88% of routable openings while embeddings alone plateau at 63%; outsourcing selection to the customer's agent (MCP tool-choice per SOP) would re-introduce the unmeasured-drift failure mode SOPilot exists to fix — MCP remains an integration *surface*, with one entry tool routed by us |
| D-12 | **Staleness gate on consumed speculation:** a pooled data item is served only if it is fresh for the CURRENT utterance. For `{user_text}`-templated deps the pooled item's embedding (its rendered query, not its content — rich fetchers prefix identical boilerplate) is compared to the live utterance embedding (already computed for rerank — no extra call); below `consume_stale_min_cos` (0.30) the runtime re-fetches live with the real query. Same-turn parallel prefetch fires the current utterance's fetch alongside classification so even turn 0 pays `max(classify, fetch)` | speculation must never *hurt*: a wrong prediction previously answered the caller's new question with last-turn's context. Found and quantified on the AENA CG A/B — prevents the "answered a question I didn't ask" failure while keeping the latency win |
| D-13 | **Two per-project reply modes** (extends D-9 `subsystems`): **advisory** = the responder gets the full SOP text + freshly-consumed data in one call, and classification/tracking/traces/prefetch run in PARALLEL, off the reply path — no verbatim drafts, no stage-gating, no rerank; **gated** (`sop`/`both`) = the full per-turn contract for procedure-critical flows. Responder model is split from the classifier (`respond_model`, e.g. a realtime/small model) — the supervisor carries the procedure, so the voice can be cheap | measured on the AENA real-customer A/B: for knowledge-delivery Q&A, advisory matched the best prompt-based agent's quality class (+SOPs is the value: +14pp coverage / +27pp concreteness) at ~40% lower median latency (background prefetch); pre-drafted verbatim replies were *net-negative* for open Q&A and are off in advisory. Gated mode remains where compliance dominates (SOPBench: +13–32pp, −57–93% skipped steps). One platform, mode chosen per procedure |
