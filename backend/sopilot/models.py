...
class ConfigAnalysisReport(Base):
    """Stage-0 System Analysis report, versioned like the ruleset. Each analysis
    run is a new immutable version; published_version is the report the schema is
    currently synced from. One "default" per project (see the design doc)."""

    __tablename__ = "config_analysis_reports"
    __table_args__ = (UniqueConstraint("tenant_id", "project_id", "name"),)
...
class ConfigAnalysisReportVersion(Base):
    __tablename__ = "config_analysis_report_versions"
    __table_args__ = (UniqueConstraint("report_id", "version"),)

    id: Mapped[str] = mapped_column(String(32), primary_key=True, default=_uuid)
    report_id: Mapped[str] = mapped_column(ForeignKey("config_analysis_reports.id", ondelete="CASCADE"), index=True)
    version: Mapped[int] = mapped_column(Integer)
    report: Mapped[dict] = mapped_column(JSON)  # the full sopilot-system-analysis document
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
